...
var entitySchema = mongoose.Schema({_id:{},attrs:{},modDate:String,creDate:String})
var Entity = mongoose.model('Entity', entitySchema)
var base_response={"_links": {"self": { "href": "" }},"measures":[{}]};
...
/*root case*/
            if(parts.length==0)
              caseId=localEnum.ROOT;

/*region case*/  
            else if(parts.length==2 && parts[0]=="monitoring" && parts[1]=="regions")
              caseId=localEnum.LIST_REGION;//monitoring/regions
...
res.writeHead(statusType, {'Content-Type': 'json'});
...
res.writeHead(errType, {'Content-Type': 'json'});
...
function getRegionList(res, statusType ,authToken){
  ...
  Entity.find({"_id.type": "region" } ,function (err, regions){
    if (regions && !(err)){
      new Date().getTime();
  ...
  if (err){
    ...
    if(1){
      ...
      for (j=0; j<region.attrs.length; j++){
        ...
        if (tmp_A.nb_ram !=0 ) tmp_A.percRAMUsed=nb_ram_used/(tmp_A.nb_ram * parseFloat(ram_allocation_ratio)); else tmp_A.percRAMUsed  =0;
        if (tmp_A.nb_disk !=0 ) tmp_A.percDiskUsed=nb_disk_used/tmp_A.nb_disk; else tmp_A.percDiskUsed=0;       
...
function getHostList(res, statusType, authToken, regionId){
  if (authToken){
   ...
   var options={
     ...
     method:"GET",
     headers:{'accept':'application/json','user-group':'none'}//end of headers
     };//end of option
           ...
           //console.log(organizations)
             for (it=0; it<organizations.length; it++){
               if(organizations[it].actorId){
                 ...
                 if (IsIO([organizations[it].id]))
                     infraName.push(organizations[it].displayName);
               }//if
             }//for
           }//if
             if( IsTRUSTED_APP(app_id) || IsAdmin(orgArray) || IsSLA(orgArray) || IsFedMan(orgArray) ){
               //console.log("ADMIN IN");
               Entity.find({$and: [{"_id.type": /host_c/ }, {"_id.id": {$regex : regionId+":*"}}] } ,function (err, host){
                 if (host && !(err)){
        ...
        try{
          ...
          sendErrorResponse (res, localEnum.UNAUTHORIZED.value, localEnum.UNAUTHORIZED.key);
           ...
           if (organizations)
             ...
             Entity.findOne({$and: [{"_id.type": /host_c/ }, {"_id.id": {$regex : regionId}}, {"attrs.value": hostId}  ] } ,function (err, host) { 
             if (host && !(err) && (IsAdmin(orgArray) || IsTRUSTED_APP(app_id)  || IsFedMan(orgArray) || IsSLA(orgArray) || (IsDev(orgArray) && host._id.type=="host_compute"  ) || (IsIO (orgArray))  && (  (infraName.indexOf((regionId)) != -1) || host._id.type=="host_compute"))){
                 ...
                 tmp_cpu=0; tmp_ram=0; tmp_disk=0;tmp_sys=0; tmp_owd=0; tmp_bwd=0; tmp_time=0;
    ...
    try{
	  ...
	  var queryString='select * from host where host_id="'+hostId+'" and UNIX_TIMESTAMP(timestampId) >= UNIX_TIMESTAMP("'+sinceValue+'");'
          ...
          if (err){
            ...
            counter=0;
            for (var t_h in rows){
              if (counter==0 &&  rows[t_h] && ((/* (IsTRUSTED_APP(orgArray) && rows[t_h].role=="compute")*/ IsTRUSTED_APP(app_id) || IsAdmin(orgArray) ||  IsFedMan(orgArray) || IsSLA(orgArray) || (IsDev(orgArray) && rows[t_h].role=="compute"  ) || (IsIO (orgArray))  && (  (infraName.indexOf((regionId)) != -1) || rows[t_h].role=="compute"   ) ))  ){
                  counter=1;
                  //console.log("OK rows")
                  tmp_res.role=rows[t_h].role;
                  tmp_res.ipAddresses[0]['ipAddress']=rows[t_h].host_name
                }
                var tmp_entry = {"timestamp" : "NaN", "percCPULoad": {"value": "NaN","description": "desc"}, "percRAMUsed": {"value": "NaN", "description": "desc" },"percDiskUsed": {"value": "NaN", "description": "desc"},  "sysUptime": { "value": "NaN", "description": "desc"},"owd_status": { "value": "NaN", "description": "desc"},"bwd_status": {"value": "NaN","description": "desc"}, "sysUptime": {"value": "NaN","description": "desc"}}
                ...
                if (rows[t_h].sysUptime)
                 tmp_entry.sysUptime.value = (rows[t_h].sysUptime);
   	 ...
   	 var actorIdArray=[]
         ...
         //if(rolesArray.indexOf(localEnum.ADMIN.value) != -1 || rolesArray.indexOf(localEnum.FED_MAN.value) != -1 ||  ){
           ...
           //console.log('admin or Fed Man IN');
            ...
            Entity.find({$and: [{"_id.type": "vm" }, {"_id.id": {$regex : filter}}] } ,function (err, vmx){
                 ...
                 if(now-vmx[l].modDate<cfgObj.vmTTL){
                   ...
                   //vmList.forEach(function(entry) {
                     //console.log(entry[0]+" vs "+ip_id)
                       tmp_vm={"_links" : {"self": { "href": "/monitoring/regions/"+region_id+"/vms/"+id_id }},"id": id_id};
                       vms.push(tmp_vm);
                   //});//end for each
                 }
                 else continue;
  ...
  if (authToken){
   ...
   var options={
     ...
     try{
       ...
       /*********/
         var tmp_res={"_links": {"self": { "href": "" }},"measures":[{}]};
         tmp_res._links.self={ "href": "/monitoring/regions/"+regionId+"/vms/"+vmId}
         tmp_res._links.services={ "href": "/monitoring/regions/"+regionId+"/vms/"+vmId+"/services"}
         actorIdArray=[]
         try{
            UserJson = eval('(' + data + ')');
	 }
         ...
         if (roles){
           ...
           // if(rolesArray.indexOf(localEnum.ADMIN.value) != -1 || rolesArray.indexOf(localEnum.FED_MAN.value) != -1){
             ...
             tmp_time='';
             tmp_sys='';
             Entity.findOne({$and: [{"_id.type": "vm" }, {"_id.id":  {$regex : vmId}   }] } ,function (err, vmValue) {
               tmp_time=''; tmp_cpu=''; tmp_ram=''; tmp_disk=''; tmp_sys='';tmp_hostname='';
               if (vmValue){
                 for (j=0; j<vmValue.attrs.length; j++){
                  ...
                  tmp_res.measures=[{"timestamp" : ""+tmp_time,
                           ...
                           "percRAMUsed": {"value": tmp_ram, "description": "desc" },
                           "percDiskUsed": {"value": (100-parseInt(tmp_disk)), "description": "desc"},
                           "hostName":{"value": tmp_host_name, "description": "desc" },
                           "sysUptime": { "value": tmp_sys, "description": "desc"}}];
                  sendResponse (res, localEnum.OK.value, tmp_res);
                       ...
                       for (j=0;j<tmpVmList.length; j++){
                         if (tmpVmList[j].split(",").length>1){
                           ...
                           if(actorIdArray.indexOf(compareID) > -1 && tmpVmList[j].indexOf(vmId)!=-1){
                             vmList.push(tmpVmList[j].split(","));
                             Entity.findOne({$and: [{"_id.type": "vm" }, {"_id.id": regionId+':'+vmId}] } ,function (err, vmValue) {
                               tmp_time=''; tmp_cpu=''; tmp_ram='';tmp_disk=''; tmp_sys='';
                               if (vmValue){
                                 for (j=0; j<vmValue.attrs.length; j++){
                                   if (vmValue.attrs[j].name && vmValue.attrs[j].name.indexOf("cpuLoadPct") != -1 && vmValue.attrs[j].value){
                                      if (vmValue.attrs[j].value>=1) tmp_cpu=100;
          ...
          if (IsJsonString(data)){
           ...
           if (roles)
             ...
             //if(rolesArray.indexOf(localEnum.ADMIN.value) != -1 || rolesArray.indexOf(localEnum.FED_MAN.value) != -1){
                 ...
                 queryString = 'select * from vm where entityId="'+regionId+'_'+vmId_+'" and UNIX_TIMESTAMP(timestampId) >=  UNIX_TIMESTAMP("'+sinceValue+'") order by timestampId';
                 connection.query(queryString, function(err, rows, fields) {
                   ArrayVM=[];
                     if (err){
                       sendErrorResponse (res, localEnum.NOT_FOUND.value, localEnum.NOT_FOUND.key)
                       ...
                       for (var i in rows) {
                         ...
                         ArrayVM.push({"timestamp" : rows[i].timestampId, "percCPULoad": {"value": cpu,"description": "desc"}, "percRAMUsed": {"value": ram, "description": "desc" },"percDiskUsed": {"value": dsk, "description": "desc"}, "availability": { "value": avl, "description": "desc"}, "sysUptime": { "value": 0, "description": "desc"}})
                        }
...
function getH2HList(res, statusType, authToken){
  Entity.find({"_id.type": "host2host" } ,function (err, host2hosts) {
    if (host2hosts && !(err)){
      var tmp_host2host=[];
      var tmp_res={"_links": {"self": { "href": "" }},"measures":[{}]};
      tmp_res._links.self={ "href": "/monitoring/host2hosts"}
      new Date().getTime();
      var now=(Math.floor(Date.now() / 1000));
      for (i=0; i<host2hosts.length;i++){
	if(now-host2hosts[i].modDate<cfgObj.h2hTTL){
          h2hId=host2hosts[i]._id.id;
          tmp_host2host.push({"_links" : { "self": { "href": "/monitoring/host2hosts/"+h2hId }},"id": h2hId})
        }//and good host
      }//end for hosts
      if (tmp_host2host.length>0)
        tmp_res.hosts=tmp_host2host;
      sendResponse (res, localEnum.OK.value, tmp_res);
    }
    else{
      sendErrorResponse (res, localEnum.NOT_FOUND.value, localEnum.NOT_FOUND.key);
...
function getH2H(res, statusType, authToken, h2hId){
  Entity.findOne({$and: [{"_id.type": "host2host" }, {"_id.id": h2hId}] } ,function (err, h2h) {
    if (h2h && !(err)){
      new Date().getTime();
      now=(Math.floor(Date.now() / 1000));
      if(now-h2h.modDate<cfgObj.h2hTTL){
        var tmp_res={"_links": {"self": { "href": "" }},"measures":[{}]};
        tmp_res._links.self={ "href": "/monitoring/host2hosts/"+h2h._id.id}
        if((h2h._id.id).split(';')[0])
          tmp_res.source=(h2h._id.id).split(';')[0];
        if((h2h._id.id).split(';')[1])
          tmp_res.dest=(h2h._id.id).split(';')[1];
        tmp_time=NaN;
        tmp_owd_m=NaN;
        tmp_owd_M=NaN;
        tmp_bw=NaN;
        ...
        for (j=0; j<h2h.attrs.length;j++){
          ...
          if (h2h.attrs[j].name && h2h.attrs[j].name.indexOf("timestamp") != -1){
            if(h2h.attrs[j].value)
              tmp_time=h2h.attrs[j].value
          }
          else if (h2h.attrs[j].name && h2h.attrs[j].name.indexOf("OWD_min") != -1){
            if(h2h.attrs[j].value)
              tmp_owd_m=h2h.attrs[j].value
          }
            ...
            if(h2h.attrs[j].value)
              tmp_bw_av=h2h.attrs[j].value
          }
          else if (h2h.attrs[j].name && h2h.attrs[j].name.indexOf("bandwidth") != -1){
            if(h2h.attrs[j].value)
              tmp_bw=h2h.attrs[j].value
          }
          else if (h2h.attrs[j].name && h2h.attrs[j].name.indexOf("sys_up") != -1){
            if(h2h.attrs[j].value)
              tmp_sys=h2h.attrs[j].value
          }
          else if (h2h.attrs[j].name && h2h.attrs[j].name.indexOf("jitter") != -1){
            if(h2h.attrs[j].value)
               tmp_jit=h2h.attrs[j].value
          }
          else if (h2h.attrs[j].name && h2h.attrs[j].name.indexOf("packet_loss") != -1){
            if(h2h.attrs[j].value)
              tmp_pl=h2h.attrs[j].value
          }
          else if (h2h.attrs[j].name && h2h.attrs[j].name.indexOf("interval") != -1){
            if(h2h.attrs[j].value)
              tmp_interval=h2h.attrs[j].value
          }
        }//end for attributes
      tmp_res.measures=[{"timestamp" : tmp_time, 
                         "owd_min": {"value": tmp_owd_m,"description": "desc"}, 
                         "owd_max": {"value": tmp_owd_M, "description": "desc" }, 
                         "bandwidth": {"value": tmp_bw, "description": "desc"},
                         "jitter": { "value": tmp_jit, "description": "desc"},
                         "packet_loss": { "value": tmp_pl, "description": "desc"},
                         "bandwidth_avg": {"value": tmp_bw_av,"description": "desc"},
                         "interval": {"value": tmp_interval,"description": "desc"}}];
...
function getServiceRegion(res, statusType, authToken, regionId){
  ...
  //Entity.find({$and: [{"_id.type": "host_service" }, {"_id.id": regionId}] } ,function (err, services) {
    ...
    novaActive=0;novaTot=0;
    cinderActive=0;cinderTot=0;
    quantumActive=0;quantumTot=0;
    qL3=0;qL3Status=0;
    qDhcp=0;qDhcpStatus=0;
    glanceActive=0;glanceTot=0;
    servActive=0;servTot=0;
    kpActive=0;kpTot=0;
    sanActive=0;sanTot=0;sanTime=0;
    if (services && !(err)){
      new Date().getTime();
      now=(Math.floor(Date.now() / 1000));
...
/*Quantum service*/
      ...
      if (glance=="green"){cnt++;tot=tot+2}
      else if (glance=="yellow"){cnt++;tot=tot+1}
      else if (glance=="red"){cnt++;}

      if (cinder=="green"){cnt++;tot=tot+2}
      else if (cinder=="yellow"){cnt++;tot=tot+1}
      else if (cinder=="red"){cnt++;}

      if (quantum=="green"){cnt++;tot=tot+2}
      else if (quantum=="yellow"){cnt++;tot=tot+1}
      else if (quantum=="red"){cnt++;}

      if (nova=="green"){cnt++;tot=tot+2}
      else if(nova=="yellow"){cnt++;tot=tot+1}
      else if (nova=="red"){cnt++;}

      if (idm=="green"){cnt++;tot=tot+2}
      else if (idm=="yellow"){cnt++;tot=tot+1}
      else if (idm=="red"){cnt++;}

      if (kp=="green"){cnt++;tot=tot+2}
      else if (kp=="yellow"){cnt++;tot=tot+1}
      else if (kp=="red"){cnt++;}
      /*
...
if (idList.indexOf((localEnum.ADMIN.value)) != -1)
  return true;
else
  return false
}
...
if (idList.indexOf((localEnum.FED_MAN.value)) != -1)
  return true;
else
  return false
}
...
if (idList.indexOf((localEnum.TrentoNode.value)) != -1   )
  return true;
else
  return false
...
if (idList.indexOf((localEnum.DEV.value)) != -1)
  return true;
else
  return false
...
if (idList.indexOf((localEnum.SLA.value)) != -1)
  return true;
else
  return false
